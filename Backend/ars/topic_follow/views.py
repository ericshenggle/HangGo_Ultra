...
class Topic_Follow_View(APIView):
    @swagger_auto_schema(
        operation_summary='根据话题id得到所有的关注者',
        responses={200: 'OK'}
    )
    def get(self, resquest, id):
        users = Topic_Subscribe.objects.filter(topic_id= id)
        serializer = Topic_Follow_Serializer(users, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)

    @swagger_auto_schema(
        operation_summary='取消关注',
        responses={204: 'OK'}
    )
    def delete(self, request, id):
        ...
        try:
            ...
            follow = Topic_Subscribe.objects.filter(topic_id=id,user_id = user_id)
            if (not DEBUG) \
        ...
        except Topic_Subscribe.DoesNotExist:
            return Response(status=status.HTTP_404_NOT_FOUND)
        return Response(status=status.HTTP_204_NO_CONTENT)


class Topic_Follow_User_View_Self(APIView):
    @swagger_auto_schema(
        operation_summary='得到用户所有订阅',
    ...
    def get(self, resquest):
        ...
        subs = Topic_Subscribe.objects.filter(user_id=user_id, topic__audit = Topic.AuditStatusChoice.审核通过)
        serializer = Topic_Follow_Serializer(subs, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)

class Topic_Follow_User_View(APIView):
    @swagger_auto_schema(
        operation_summary='得到指定用户所有订阅',
        responses={201: comment_response}
    )
    def get(self, resquest, id):
        user_id = id
...
class Topic_Follow_One_View(APIView):
    @swagger_auto_schema(
        operation_summary='关注话题',
    ...
    def post(self, resquest):
        ...
        try:
            ...
            return Response({}, status=status.HTTP_404_NOT_FOUND)
        tf = Topic_Subscribe.objects.filter(topic_id= data['topic_id'], user_id = data['user_id'])
        if tf:
            return Response(status=status.HTTP_400_BAD_REQUEST)
        topic_follow = Topic_Subscribe(**data)
        topic_follow.save()
        serializer = Topic_Follow_Serializer(topic_follow)
